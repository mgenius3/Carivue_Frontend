...
export function Navbar({ 
  ...
  role = "Executive", 
  userName = "Helen Anthony",
  orgName = "Meridian Care Group",
  onAddOrg
}: NavbarProps) {
            ...
            {isOrgOpen && (
              <div className="absolute top-full right-0 mt-2 w-56 bg-white rounded-xl shadow-xl border border-gray-100 py-2 animate-fade-in-up z-50">
                <div className="px-4 py-2 bg-[#1F3A4A]/5 border-l-4 border-[#1F3A4A]">
                  <span className="text-sm font-bold text-[#1F3A4A]">{orgName}</span>
                </div>
                <div className="px-4 py-2 hover:bg-gray-50 cursor-pointer transition-colors">
                  <span className="text-sm font-medium text-gray-500">Grace Care Home</span>
                </div>
          ...
          <div className="relative" ref={profileRef}>
            ...
            >
              ...
              <div className="text-left hidden md:block">
                <p className="text-xs font-bold text-primary group-hover:text-secondary transition-colors leading-tight">{userName}</p>
